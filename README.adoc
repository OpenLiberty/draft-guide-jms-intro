...
The `system` microservice is the producer of the messages that are published to the Jakarta messaging system as a stream of events. Every 15 seconds, the `system` microservice publishes an event that contains its calculation of the recent CPU usage for the last minute.
...
* [hotspot=testNonEmpty file=0]`testNonEmpty()` verifies that the hostname and the system load for each system in the inventory are not empty.

* [hotspot=testValue file=0]`testValue()` verifies that the hostname and system load returned by the `system` service match the ones stored in the `inventory` service and ensures they are not empty.
