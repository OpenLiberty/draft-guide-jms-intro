...
Refine the [hotspot=jmsQueue file=3]`jmsQueue` and [hotspot=jmsActivationSpec file=3]`jmsActivationSpec` configurations with the variables for IBM MQ settings. Add [hotspot=resourceAdapter file=3]`resourceAdapter` element to provide the RAR file that provides the IBM MQ classes for Java and JMS.
