...
The [hotspot=messaging file=1]`messaging` feature enables the configuration of resource adapters to access messaging systems by using the Jakarta Messaging API. The [hotspot=messagingClient file=1]`messagingClient` feature enables the application with access to the message queues that are hosted on Liberty through the Jakarta Messaging API.  The [hotspot=messagingServer file=1]`messagingServer` feature enables an embedded messaging server to listen the message queues.

Add the [hotspot=wasJmsEndpoint file=1]`wasJmsEndpoint` element to ensure that the Liberty runtime is properly configured to listen and manage incoming JMS connections from any hosts. Set up the [hotspot=messagingEngine file=1]`messagingEngine` configuration to ensure that the Liberty runtime can manage incoming message queues more effectively, assigning a reliable and persistent destination for the `InventoryQueue`. Configure a [hotspot=jmsQueueConnectionFactory file=1]`jmsQueueConnectionFactory` element to use the [hotspot=connectionManager file=1]`InventoryCM` and set properties for the JMS implementation. Define a [hotspot=jmsQueue file=1]`jmsQueue` element for the inventory system and a [hotspot=jmsActivationSpec file=1]`jmsActivationSpec` element to configure properties including the destination queue reference, destination type, and maximum concurrency.
...
The `system` microservice is the producer of the messages that are published to the Jakarta messaging system as a stream of events. Every 15 seconds, the `system` microservice publishes an event that contains its calculation of the recent system load (CPU usage) for the last minute.
...
* [hotspot=testNonEmpty file=0]`testNonEmpty()` verifies that the hostname and the CPU load for each system in the inventory are not empty.

* [hotspot=testValue file=0]`testValue()` verifies that the hostname and CPU load returned by the system service match the ones stored in the inventory service and ensures they are not empty.
...
--
If you’re a Mac user, check out the https://community.ibm.com/community/user/integration/blogs/richard-coppen/2023/06/30/ibm-mq-9330-container-image-now-available-for-appl[How to build Mac IBM MQ container image^] blog in the IBM TechXchange Community website for building IBM MQ container image.
...
Refine the [hotspot=jmsQueue file=1]`jmsQueue` and [hotspot=jmsActivationSpec file=1]`jmsActivationSpec` configurations with the variables for IBM MQ settings. Add [hotspot=resourceAdapter file=1]`resourceAdapter` element to provide the RAR file that provides the IBM MQ classes for Java and JMS.
