...
Refine the [hotspot=jmsQueue file=3]`jmsQueue` and [hotspot=jmsActivationSpec file=3]`jmsActivationSpec` configurations with the variables for IBM MQ settings.
...
Go to the command shell running `inventory` dev mode and press `enter/return` and if the tests pass, you'll see similar output to the following example:
...
//stopping dev mode
After you are finished checking out the application, stop the Liberty instance first by pressing `CTRL+C` in each command-line sessions where you ran `system` and `inventory` services. Alternatively, you can run the `liberty:stop` goal from the `finish` directory in another shell session:
[role='command']
----
mvn -pl inventory liberty:stop
mvn -pl system liberty:stop
